...
wx based GUI for reading ledenExport.csv files provided by ScoutsOnline (SOL)
Additionally a *.ini file can be imported wiht the relevant contribution
values. The program then returns per adress a contribution letter for each
member of the group which is required to pay contribution fees.
...
class WorkerThread(threading.Thread):
    ...
    def run(self):
        ...
        if self.action == 'cont':
            ...
            c_obj = Contribution(cd=self._notify_window.contributie,
                                 hf='',
                                 od=self._notify_window.odc)
            a_obj = c_obj.create(self._notify_window.address_list)
...
class SolGUI(wx.Frame):
    ...
    def InitUI(self):
        ...
        # Progress bar
        self.maxPercent = 100
        self.cp = wx.Gauge(self.panel, range=100, size=(350, 25),
        ...
        # Progress bar
        self.maxPercent = 100
        self.fp = wx.Gauge(self.panel, range=100, size=(350, 25),
                           style=wx.GA_HORIZONTAL | wx.GA_SMOOTH)
    ...
    def OpenWndw(self, event):
        secondWindow = AdressGUI(parent=None, id=-1,
                                 title="Voeg administratie kosten toe")
        secondWindow.Show()
    ...
    def open_ledenexport(self, event):
        """Read ledenexport file to memory"""
    ...
    def open_contributie(self, event):
        """Read contribution settings to memory"""
        fd = wx.FileDialog(self.panel,
                           "Open Contributie",
                           self.app_path,
                           "contributie.ini",
...
class AdressGUI(wx.Dialog):
    def __init__(self, parent):
        # Create a frame
        wx.Frame.__init__(self, None, -1, "Voeg administratie kosten toe",
                          size=(600, 200),
                          style=wx.DEFAULT_FRAME_STYLE ^ wx.RESIZE_BORDER)
        ...
        address_list = members.group(filter_list=members.addresses,
                                     key='lid_adres')
        ...
        panel2 = wx.lib.scrolledpanel.ScrolledPanel(self, -1,
                                                    size=(585, 130),
                                                    pos=(0, 30),
