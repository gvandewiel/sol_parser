...
class Contribution():
    ...
    def create(self, address_list):
        ...
        self.spdf.output(os.path.join(
            self.output_dir, 'Samenvatting.pdf'), 'F')
    ...
    def create_single(self, adres, members, aac):
        ...
        for lid in self.members:
            ...
            if 'jeugdlid *' in normalize(lid.functie):
                # If the boolean value bjeuglid is False then no member on
                # address is found that is required to pay contribution.
                if not self.bjeugdlid:
                    ...
                    self.pdf.set_y(65)
                    self.pdf.cell(w=0, h=8, ln=1, align='C',
                                  txt='Contributie Don Garcia Moreno')
                    ...
                    self.pdf.cell(w=40, h=6, ln=0, align='L',
                                  txt='Notanummer:')
                    self.pdf.cell(w=0, h=6, ln=1, align='R',
                                  txt='{}{}{:03}'.format(lid.season_start,
                                                         lid.season_end,
                                                         type(self).iNotanumber))
                    self.pdf.font()
                    self.pdf.cell(w=0, h=8, txt='', ln=1)
                    self.pdf.cell(w=100, h=6, ln=1, align='L',
                                  txt='Nota voor de ouder(s)/verzorger(s) van:')
                ...
                print('{:<35}{:<20}{:>6.2f}'.format(
                    lid.naam, lid.speleenheid, self.s_contr))

                if self.hf != '':
                    ad = ad + \
                        self.sf.accordion_data(
                            lid.naam, lid.speleenheid, self.s_contr)

                self.pdf.cell(w=100, h=6, txt=lid.naam, ln=0, align='L')
                self.pdf.cell(w=40, h=6, ln=0, align='L',
                              txt=lid.speleenheid.capitalize())

                self.pdf.cell(w=0, h=6, ln=1, align='R',
                              txt='{:.1f}'.format(self.s_contr))

                # Print paying member in summary sheet
                self.spdf.cell(w=75, h=6, txt=lid.naam, ln=0, align='L')
                self.spdf.cell(w=70, h=6, ln=0, align='L',
                               txt=lid.speleenheid.capitalize())

                self.spdf.cell(w=30, h=6, ln=0, align='L',
                               txt=lid.functie.capitalize())

                self.spdf.cell(w=0, h=6, ln=1, align='R',
                               txt='{:.1f}'.format(self.s_contr))
        ...
        if self.bjeugdlid:
            ...
            if aac:
                self.pdf.cell(w=100, h=6, ln=0, align='L',
                              txt='Adminstratiekosten')

                self.pdf.cell(w=40, h=6, txt='', ln=0, align='L')
                self.pdf.cell(w=0, h=6, ln=1, align='R',
                              txt='{:.1f}'.format(self.cd['adminstratiekosten']))

                self.t_contr += self.cd['adminstratiekosten']

                self.spdf.set_xy(25, self.spdf.ll)
                self.spdf.cell(w=75, h=6, ln=0, align='L',
                               txt='Adminstratiekosten')
            ...
            self.spdf.cell(w=0, h=6, border='B',
                           txt=lid.e_mailadres, align='L', ln=1)

            self.spdf.set_xy(100, self.spdf.al)
            print(self.t_contr)
            self.spdf.cell(w=0, h=6, border='B', align='R', ln=1,
                           txt='{:.1f}'.format(self.t_contr))
            ...
            self.spdf.cell(w=0, h=6, border='B', txt='{}'.format(
                self.t_contr), align='R', ln=1)
            self.spdf.font(style='', size=9)
