...
class Parser():
    def __init__(self, csvfile):
        ...
        with open(csvfile) as csvfile:
            try:
                ...
                for row in reader:
                    row['leeftijd'] = calculate_age(row['Lid geboortedatum'], date.today())
                    row['chk_leeftijd'] = calculate_age(row['Lid geboortedatum'], self.chk_date)
    ...
    def turnover_date():
        ...
        if turnover_date.month >= 9 and turnover_date.day >= 1:
            turnover_date = turnover_date + relativedelta(day=1, month=9, years=1)
        else:
