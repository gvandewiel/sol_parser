...
# Prevent making .pkl files of included fonts
...
class PDF(fpdf.FPDF):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Remove existing .pkl files from resource_path
        for f in os.listdir(self.fp):
            if os.path.isfile(os.path.join(self.fp, f)) and os.path.splitext(f)[1] == ".pkl":
                os.remove(os.path.join(self.fp, f))
    ...
    def header(self):
        """PDF Header."""
        # Logo
        self.image(os.path.join(self.dp, 'Scouting_NL_logo_RGB.jpg'),
                   x=self.w - 38.1 - 25, y=12.6, w=38.1, h=35.7)
        self.image(os.path.join(self.dp, 'Logo.png'),
                   x=25, y=12.6, w=38.1, h=35.7)
        self.add_font('DejaVuSans', '', os.path.join(
            self.fp, 'DejaVuSans.ttf'), uni=True)
        self.add_font('DejaVuSans', 'B', os.path.join(
            self.fp, 'DejaVuSans-Bold.ttf'), uni=True)
        self.add_font('DejaVuSans', 'I', os.path.join(
            self.fp, 'DejaVuSans-Oblique.ttf'), uni=True)
    ...
    def nota_end(self, aac=True):
        ...
        self.font()
        self.cell(w=0, h=6, align='L', ln=1,
                  txt='Namens Stichting Scouting Middelbeers,')
        self.cell(w=0, h=6, align='L', ln=1, txt='D. Schima, Penningmeester')
        self.cell(w=0, h=6, align='L', ln=1, txt='')
        if not aac:
            self.multi_cell(
                w=0, h=6, align='L', txt='U hoeft geen verdere actie te ondernemen. Het bedrag wordt automatisch, middels automatisch incasso, in 2 delen van uw rekening afgeschreven.')
        else:
            self.cell(w=0, h=6, align='L', ln=1,
                      txt='Gelieve het totaalbedrag voor 1 februari over te boeken op bankrekeningnummer:')
            self.font(style='B')
            self.cell(w=0, h=6, align='C', ln=1, txt='NL45RABO0133812006')
            self.font()
            self.cell(w=0, h=6, align='L', ln=1,
                      txt='t.n.v. Stichting Scouting Don Garcia Moreno te Middelbeers.')
            self.cell(w=0, h=6, align='L', ln=1,
                      txt='Bij betaling graag het notanummer vermelden!')
            self.cell(w=0, h=6, align='L', ln=1, txt='')
            self.font(style="I")
            self.multi_cell(
                w=0, h=6, align='L', txt='Mocht u nog niet gekozen hebben voor automatische incasso kunt u hiervoor alsnog een machtigingsformulier opvragen via het mailadres:')
            self.cell(w=0, h=6, align='C', ln=1,
                      txt='dieterschima@scoutingdongarciamoreno.nl')
            self.font()
        ...
        self.cell(w=0, h=6, align='L', ln=1,
                  txt='Penningmeester Scouting Don Garcia Moreno')

    def footer(self):
        """PDF Footer."""
        self.image(os.path.join(self.dp, 'SN_Blad_small.jpg'),
                   x=0, y=self.h - 38.2, w=self.w, h=38.2)

        self.font(size=9)
        self.set_y(-30)
        self.cell(w=0, h=5, ln=1, align='C',
                  txt='Scouting Don Garcia Moreno - Konijnenberg 2 - 5091 TS Oost West en Middelbeers')
        self.cell(w=0, h=5, ln=1, align='C',
                  txt='tel +31 (0)13 514 17 66 - e-mail contributie@scoutingdongarciamoreno.nl - web www.scoutingdongarciamoreno.nl')
    ...
    def f_line(self, item, w=0, ln=1):
        ...
        if self.member is not None:
            if item[0] == '':
                ...
                try:
                    self.cell(w=w, h=6, txt=str(
                        getattr(self.member, item[1])), ln=ln, align='L')
...
class Summary_Sheet():
    ...
    def accordion_start(self, lid):
        notanumber = '{}{}{:03}'.format(
            lid.season_start, lid.season_end, type(self.NotaObj).iNotanumber)
        ...
        self.sf.write(
            '                                <div class="post panel panel-default margin-0" id="post_{}">'.format(notanumber))
        ...
        self.sf.write('                                    </div>\n')
        self.sf.write(
            '                                    <div id="collapse_{}" class="panel-collapse collapse" style="height: 0px;">\n'.format(notanumber))
        self.sf.write(
